...
package com.dev.ops.defa.notifiers;
...
public class BroadcastNotifier {
     ...
     * @param result
     */
    public void broadcastIntentWithState(int status, String broadcastAction, String result) {

        Intent localIntent = new Intent(broadcastAction);

        // Puts the status into the Intent
        localIntent.putExtra(CommonPreferences.Notifiers.EXTENDED_DATA_STATUS, status);
        if(result!=null) {
            localIntent.putExtra(CommonPreferences.Notifiers.EXTENDED_DATA_RESULT, result);
        }
       ...
       /* // Notify UI that registration has completed, so the progress indicator can be hidden.
        Intent registrationComplete = new Intent(CommonPreferences.BROADCAST_ACTION);
        registrationComplete.putExtra("outparam", "Rohan");
